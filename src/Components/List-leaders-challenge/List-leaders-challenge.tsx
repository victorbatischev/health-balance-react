...
import { TEAM_MEMBERS_PAGE } from "../../provider/constants-route";
import { IListLeadersChallengeItem } from "../../types/interfaces";
          ...
          <NavLink to={TEAM_MEMBERS_PAGE + "/" + item.id}>
            <article className="leader-challenge__item item-leader" key={i}>
              <div className="item-leader__column item-leader__column_1">
                <div
                  className={"item-leader__place " + colorReward(item.place)}
                >
                  {item.place}
                </div>
                <div
                ...
                >
                  {item.title}
                  {item.isYourCommand && (
                    <div className="item-leader__your-command">
                      Ваша команда
                    </div>
                  )}
                </div>
